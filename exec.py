...
TEXT_LENGTH_LIMIT = 300000
DEF_ENDPOINT = "https://openai.lcl.offis.de/"
LOC_ENDPOINT = "http://localhost:11434/api/"


def run_with_new_instance(
    client, prompt, text, model="llama3.3:70b", temperature=1.5, sleep=0, seed=1313
):
    ...
    reset_response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "user",
                "content": "Forget about previous inquiries, instructions and prompts.",
            }
        ],
    )
    if sleep:
    ...
    completion = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "user",
                "content": f"Here's a piece of text, followed by instructions: “{text[:TEXT_LENGTH_LIMIT]}”.\n{prompt}",
            },
        ],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ver", default="v5", type=str)
    parser.add_argument("--code", default=None, type=str)
    parser.add_argument(
        "--model", default="deepseek-r1:70b", type=str
    )  #'gpt-oss:120b', 'llama3.3:70b', deepseek-r1:70b
    parser.add_argument("--temp", default=1.5, type=float)
    parser.add_argument("--seed", default=1313, type=int)
    parser.add_argument("--endpoint", default="loc", type=str)
    parser.add_argument(
        "--subset",
        default=["ru"],  # 'ru',
        type=str,
    ...
    if args.endpoint == "def":
        endpoint = DEF_ENDPOINT
        apikey = read_apikey(os.path.join(base_dir, "def_apikey.key"))
    else:
        endpoint = LOC_ENDPOINT
        apikey = ""
    ...
    def run_proccess(subset, code, desc):
        ...
        result_file = os.path.join(
            results_dir,
            f"{args.model}_{args.seed}_{args.temp}_{args.ver}_{subset}_{code}_{args.test}".replace(
                ":", "-"
            ),
        )
        ...
        if n_done < n:
            ...
            for idx, item in tqdm(
                ...
                total=n,
                leave=False,
                desc=f"{code}/{args.model}",
            ):
                try:
                    text = item["text_raw"]
                    output = run_with_new_instance(
                        client,
                        prompt,
                        text,
